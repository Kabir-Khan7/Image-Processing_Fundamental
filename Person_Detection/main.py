# yolo_person_detection.py – Accurate person detection using YOLOv8
import cv2
import os
...
# 1. CONFIG
# -------------------------------------------------
VIDEO_IN   = "people2.mp4"  # Download a free video from Pexels/Pixabay (e.g., crowded street)
VIDEO_OUT  = "output_yolo_persons.mp4"
CONF_THRESH = 0.5   # Confidence threshold (0.3–0.7; lower for detecting distant/small persons)
...
# -------------------------------------------------
# 4. Person class ID in COCO (YOLOv8)
# -------------------------------------------------
PERSON_CLASSES = {
    0: "person"
}
...
while True:
    ...
    if boxes is not None:
        for i, box in enumerate(boxes):
            ...
            label = PERSON_CLASSES.get(cls_id, "person")
            cur_persons += 1
            total_persons += 1
            # Draw
    ...
    cv2.putText(frame, f"Persons: {cur_persons}", (10, 60),
    ...
    cv2.imshow("YOLOv8 Person Detection – Press Q", disp)
...
avg = total_persons / frame_idx if frame_idx else 0
print(f"\n{'='*60}")
